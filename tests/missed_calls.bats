...
@test "parse_missed_call_line extracts number and datetime" {
    run parse_missed_call_line 'Call 1, Missed, Number "+393755403326", Date/time: 21.10.2025 15:02:00'
    [ "$status" -eq 0 ]
    [ "$output" = "+393755403326|21.10.2025_15:02:00" ]
}

@test "parse_missed_call_line rejects line without datetime" {
    run parse_missed_call_line 'Call 1, Missed, Number "+393755403326"'
    [ "$status" -eq 1 ]
}
...
@test "two distinct calls from same number both publish" {
    stub_gammu_calllog 'Call 1, Missed, Number "+393755403326", Date/time: 21.10.2025 15:02:00
Call 2, Missed, Number "+393755403326", Date/time: 21.10.2025 15:55:00'
    stub_mosquitto_pub
    check_missed_calls
    [ "$(publish_count 'Missed call from: +393755403326')" -eq 2 ]
}
